# Project Alpha — 默认种子数据（SQL Server）

- **规模**：`tag` **15** 条（平台 / 体系 / 场景，风格类似 **ISO、Android、Windows**），`ticket` **500** 条（中文标题与描述由模板轮换生成，并在末尾附加 **唯一流水号**，保证 **500 条标题与 500 条描述互不重复**），`ticket_tag` 每条 ticket **1～4** 个标签（去重，与 15 个 tag 同步）。
- **表结构**：须与 `specs/w1/0001-spec.md` 第 5.3 节 DDL 一致。
- **用法**：在 SSMS / `sqlcmd` 中先 `USE` 目标库，再执行下方脚本。脚本会**清空** `ticket_tag`、`ticket`、`tag` 并重置 `ticket` / `tag` 的 IDENTITY。

> 同内容纯 SQL：`specs/w1/0003-seed-data.sql`
...
/* ========== 关联：每条 ticket 最多 4 个 tag（id 映射到 1～15） ========== */
...
SELECT (SELECT COUNT(*) FROM tag) AS tags,
       ...
       (SELECT COUNT(*) FROM ticket_tag) AS links;
*/
```
