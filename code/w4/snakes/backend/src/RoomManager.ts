import { Room, RoomInfo, BattlePlayer, Position, Food, RoomStatus } from './types';
...
export class RoomManager {
  ...
  private playerIdToSocket: Map<string, string> = new Map(); // playerId -> socketId
  ...
  leaveRoom(socketId: string): Room | null {
    ...
    // 找到并移除玩家
    let removedPlayer: BattlePlayer | undefined;
   ...
   */
  findRecoverablePlayer(playerName: string): { room: Room; player: BattlePlayer } | null {
   ...
   * 恢复玩家连接
   */
  recoverPlayer(player: BattlePlayer, newSocketId: string): void {
    // 更新socket映射
