  ...
  socket.on('message', async (message: ClientMessage) => {
    try {
      switch (message.type) {
        ...
        case 'createRoom':
          handleCreateRoom(socket, message.data as CreateRoomData);
          break;

        case 'joinRoom':
          handleJoinRoom(socket, message.data as JoinRoomData);
          break;
        ...
        case 'playerMove':
          handlePlayerMove(socket, message.data.direction as Direction);
          break;

        case 'useItem':
          handleUseItem(socket, message.data.itemId as string, message.data.targetPlayerId as string);
          break;
...
function handleCreateRoom(socket: any, data: CreateRoomData) {
...
function handleJoinRoom(socket: any, data: JoinRoomData) {
...
function handleResetRooms(socket: any) {
  console.log('🔄 Resetting all rooms');
  roomManager.reset();
  broadcastRoomList();
}

function handleRequestPause(socket: any) {
...
function handleConfirmPause(socket: any) {
  const room = roomManager.getRoomBySocketId(socket.id);
  if (!room || room.status !== 'playing' || !room.pauseRequest || room.isPaused) return;
...
function handleRequestResume(socket: any) {
  const room = roomManager.getRoomBySocketId(socket.id);
  if (!room || room.status !== 'playing' || !room.isPaused) return;
...
function handleConfirmResume(socket: any) {
  const room = roomManager.getRoomBySocketId(socket.id);
  if (!room || room.status !== 'playing' || !room.pauseRequest || !room.isPaused) return;
...
function handleExitGame(socket: any) {
  const room = roomManager.getRoomBySocketId(socket.id);
  if (!room) return;
...
function handleToggleReady(socket: any) {
...
function handleStartGame(socket: any) {
  const room = roomManager.getRoomBySocketId(socket.id);
  if (!room || room.status !== 'waiting') return;
...
function handlePlayerMove(socket: any, direction: Direction) {
  const room = roomManager.getRoomBySocketId(socket.id);
...
function handleUseItem(socket: any, itemId: string, targetPlayerId: string) {
  const room = roomManager.getRoomBySocketId(socket.id);
  if (!room || room.status !== 'playing') return;
...
function handleSelectMap(socket: any, map: any) {
