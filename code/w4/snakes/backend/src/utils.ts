import { Position, BattleItemType } from './types';
 ...
 * 检查位置是否碰撞
 */
export function checkCollision(pos: Position, positions: Position[], gridWidth: number, gridHeight: number, walls: Position[] = [], obstacles: Position[] = []): boolean {
  // 检查边界
 ...
 * 根据权重随机选择道具类型
 */
export function getRandomItemType(): BattleItemType | 'normal' {
  ...
  for (const [type, weight] of Object.entries(ITEM_WEIGHTS)) {
    random -= weight;
    if (random <= 0) {
      return type as BattleItemType | 'normal';
    }
  }
 ...
 * 获取蛇的初始位置
 */
export function getInitialSnakePosition(index: number, gridWidth: number, gridHeight: number): Position[] {
